...
- `decompile-analysis [--file FILE] 0xADDR` emits structured decompiler facts for automation: recovered CFG diagnostics, reducibility/goto-pressure metrics, stack/call/syscall facts, and first-pass register SSA/data-flow facts.
- `decompiler-benchmark [--file FILE | --root DIR] [--max-functions N]` scores recovery, pseudo-C, CFG reducibility, machine-fact coverage, and register data-flow coverage with blockers for production-grade decompiler work.
