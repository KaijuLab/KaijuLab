...
- `decompile-analysis [--file FILE] 0xADDR` emits structured decompiler facts for automation: recovered CFG diagnostics, reducibility/goto-pressure metrics, stack/call/syscall facts, first-pass register SSA/data-flow facts, and stack/global variable candidates.
- `decompiler-benchmark [--file FILE | --root DIR] [--max-functions N]` scores recovery, pseudo-C, CFG reducibility, machine-fact coverage, register data-flow coverage, and memory-variable fact coverage with blockers for production-grade decompiler work.
