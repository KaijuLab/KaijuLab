...
kaijulab api agent-decompile-loop claude --file ./foo.bin --function 0x401000
...
kaijulab api recovery-stored --file ./foo.bin
kaijulab api recovery-correct --file ./foo.bin --action rename_function --vaddr 0x401000 --note main
...
- `decompile-analysis [--file FILE] 0xADDR` emits structured decompiler facts for automation: recovered CFG diagnostics, reducibility/goto-pressure metrics, KIR semantic ops, KIR SSA facts, stack/call/syscall facts, first-pass register SSA/data-flow facts, and stack/global variable candidates.
- `agent-decompile-loop <claude|codex> --function 0xADDR` sends Agent Console a bounded decompiler-analysis prompt with enhanced decompile text, compact structured KIR/SSA/type/call facts, quality blockers, graph-backed xref/CFG commands, runtime/debug verification commands, and a JSON final-report contract.
- `decompiler-benchmark [--file FILE | --root DIR] [--max-functions N]` scores recovery, pseudo-C, CFG reducibility, KIR/KIR-SSA coverage, machine-fact coverage, register data-flow coverage, and memory-variable fact coverage with blockers for production-grade decompiler work.
...
- `recovery-stored [--file FILE]` loads persisted recovery facts, rebuilding if no recovery DB exists yet.
- `recovery-correct --action <rename_function|mark_code|mark_data|split_function|merge_function> --vaddr 0xADDR` records an analyst correction and applies it to persisted recovery facts.
