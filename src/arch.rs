...
impl ArchClass {
    ...
    pub fn from_object(arch: Architecture) -> Self {
        match arch {
            ...
            Architecture::Mips => ArchClass::Mips {
                bits: 32,
                big_endian: true,
            },
            Architecture::Mips64 => ArchClass::Mips {
                bits: 64,
                big_endian: true,
            },
            Architecture::Riscv32 => ArchClass::RiscV { bits: 32 },
            Architecture::Riscv64 => ArchClass::RiscV { bits: 64 },
            Architecture::PowerPc => ArchClass::PowerPc {
                bits: 32,
                big_endian: true,
            },
            Architecture::PowerPc64 => ArchClass::PowerPc {
                bits: 64,
                big_endian: true,
            },
            _ => ArchClass::Unknown,
...
pub fn build_capstone(arch: ArchClass) -> Result<capstone::Capstone, String> {
    ...
    let cs = match arch {
        ArchClass::Arm64 => Capstone::new()
            .arm64()
            .mode(cs_arch::arm64::ArchMode::Arm)
            .build(),
        ArchClass::Arm => Capstone::new()
            .arm()
        ...
        ArchClass::Mips { bits, big_endian } => {
            let mode = if bits == 64 {
                cs_arch::mips::ArchMode::Mips64
            } else {
                cs_arch::mips::ArchMode::Mips32
            };
        ...
        ArchClass::RiscV { bits } => {
            let mode = if bits == 64 {
                cs_arch::riscv::ArchMode::RiscV64
            } else {
                cs_arch::riscv::ArchMode::RiscV32
            };
            Capstone::new().riscv().mode(mode).build()
        }
        ArchClass::PowerPc { bits, big_endian } => {
            let mode = if bits == 64 {
                cs_arch::ppc::ArchMode::Mode64
            } else {
                cs_arch::ppc::ArchMode::Mode32
            };
            let endian = if big_endian {
                Endian::Big
            } else {
        ...
        other => return Err(format!("Use iced-x86 for {}, not Capstone", other.label())),
    };
...
pub fn is_direct_branch(arch: ArchClass, mnemonic: &str, op_str: &str) -> bool {
    if is_direct_call(arch, mnemonic, op_str) {
        return true;
    }
    let m = mnemonic.trim().to_ascii_lowercase();
    match arch {
...
pub fn is_return(arch: ArchClass, mnemonic: &str, op_str: &str) -> bool {
    ...
    match arch {
        ArchClass::Arm64 => {
            m == "ret" || (m == "br" && o.contains("x30")) || (m == "b" && o.contains("lr"))
            // rare
        }
        ArchClass::Arm => {
            (m == "bx" && o.contains("lr"))
...
pub fn matches_prologue(bytes: &[u8], patterns: &[([u8; 4], [u8; 4])]) -> bool {
    ...
    patterns
        .iter()
        .any(|(pat, mask)| (0..4).all(|i| mask[i] == 0x00 || bytes[i] == pat[i]))
}
...
pub const AARCH64_REGS: &[&str] = &[
    ...
    "x28", "x29", "x30", // 32-bit GP (lower halves)
    "w0", "w1", "w2", "w3", "w4", "w5", "w6", "w7", "w8", "w9", "w10", "w11", "w12", "w13", "w14",
    "w15", "w16", "w17", "w18", "w19", "w20", "w21", "w22", "w23", "w24", "w25", "w26", "w27",
    "w28", "w29", "w30", // Special-purpose
    "sp", "lr", "fp", "xzr", "wzr", "pc", // SIMD / FP scalar
    "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14",
    "v15", "q0", "q1", "q2", "q3", "q4", "q5", "q6", "q7", "d0", "d1", "d2", "d3", "d4", "d5",
    "d6", "d7", "s0", "s1", "s2", "s3", "s4", "s5", "s6", "s7", "h0", "h1", "h2", "h3", "b0", "b1",
    "b2", "b3",
];

pub const ARM32_REGS: &[&str] = &[
    "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7", "r8", "r9", "r10", "r11", "r12", "r13", "r14",
    "r15", "sp", "lr", "pc", "fp", "ip", "sl", "sb", // NEON / VFP (abbreviated)
...
pub const RISCV_REGS: &[&str] = &[
    ...
    "t4", "t5", "t6", // Numeric aliases
    "x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9", "x10", "x11", "x12", "x13", "x14",
    "x15", "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27",
    "x28", "x29", "x30", "x31", // FP
    "ft0", "ft1", "ft2", "ft3", "ft4", "ft5", "ft6", "ft7", "fa0", "fa1", "fa2", "fa3", "fa4",
    "fa5", "fa6", "fa7", "fs0", "fs1", "fs2", "fs3", "fs4", "fs5", "fs6", "fs7", "fs8", "fs9",
    "fs10", "fs11",
];
...
pub const PPC_REGS: &[&str] = &[
    ...
    "r28", "r29", "r30", "r31", "f0", "f1", "f2", "f3", "f4", "f5", "f6", "f7", "lr", "ctr", "xer",
    "msr", "pvr", "cr0", "cr1", "cr2", "cr3", "cr4", "cr5", "cr6", "cr7", "sp", "rtoc",
];
...
pub const AARCH64_LIB_SIGS: &[(&str, &[u8], usize)] = &[
    ...
    (
        "memcpy",
        &[0xc8, 0x0b, 0x00, 0xd1, 0xe8, 0x03, 0x02, 0xaa],
        8,
    ),
    // memset (glibc): and w3, w2, w2 ... cmp
    (
        "memset",
        &[0x03, 0x00, 0x80, 0x52, 0x1f, 0x00, 0x00, 0xea],
        8,
    ),
    // strlen: ldrb + cbz + add loop
    (
        "strlen",
        &[0x00, 0x00, 0x40, 0x39, 0x00, 0x00, 0x00, 0xb4],
        8,
    ),
    // strcmp
    (
        "strcmp",
        &[0x02, 0x00, 0x40, 0x39, 0x22, 0x00, 0x40, 0x39],
        8,
    ),
    // __libc_start_main
    (
    ...
    (
        "_start",
        &[0x1d, 0x00, 0x80, 0xd2, 0x3e, 0x00, 0x80, 0xd2],
        8,
    ),
];
