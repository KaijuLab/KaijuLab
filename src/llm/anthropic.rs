...
/// Convert universal history to Anthropic's `messages` array.
///
/// Anthropic uses `role: "user" | "assistant"` only.  Tool results go back
/// as `role: "user"` with `tool_result` content blocks.
