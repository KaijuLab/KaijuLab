...
//!   - Any other OpenAI-compatible server (LM Studio, vLLM, etc.)
...
/// Outgoing messages for the `/chat/completions` endpoint.
/// Built as raw JSON Values to handle the tool / tool-call split cleanly.
fn to_oai_messages(system: &str, history: &[LlmMessage]) -> Vec<serde_json::Value> {
    ...
    for msg in history {
        match msg.role {
            MessageRole::User => {
                // Split: text parts → one "user" message; tool results → one "tool" message each
            ...
            MessageRole::Assistant => {
                ...
                let texts = msg.texts();
                let content_str: Option<String> = if texts.is_empty() {
                    None
                } else {
                    Some(texts.join("\n"))
                };
...
impl OpenAiBackend {
    ...
    ///
    /// - `api_key`: pass an empty string for Ollama (no auth needed).
...
impl LlmBackend for OpenAiBackend {
    async fn generate(
        ...
        let oar: OAResponse = resp.json().await.context("Failed to parse OpenAI response")?;
        let msg = oar
            .choices
            .into_iter()
            .next()
        ...
        // Translate to universal LlmMessage
    ...
    fn display_name(&self) -> String {
        // Distinguish Ollama vs OpenAI by base URL
