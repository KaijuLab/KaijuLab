...
/// `#[serde(untagged)]`: variant order matters — most-specific first.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(untagged)]
enum GPart {
    FunctionCall {
...
struct GRequest<'a> {
    ...
    // Unused fields from parent struct aren't needed
    #[serde(skip)]
...
/// Gemini requires uppercase JSON Schema type names ("OBJECT", "STRING", …).
...
fn from_gemini_candidate(candidate: GCandidate) -> LlmMessage {
    ...
    let content: Vec<MessageContent> = candidate
        ...
        .filter_map(|(i, part)| match part {
            GPart::Text { text } => Some(MessageContent::Text(text)),
            GPart::FunctionCall { function_call } => Some(MessageContent::ToolCall(ToolCall {
                // Gemini has no call IDs; use name + index to keep them unique
